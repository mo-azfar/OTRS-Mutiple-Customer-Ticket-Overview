...
package Kernel::Modules::CustomerTicketOverview2;
...
sub Run {
    ...
    if ( !$Self->{Subaction} ) {
        return $LayoutObject->Redirect(
            OP => 'Action='.$Self->{Action}.';Subaction=MyTickets',
        );
    }
	...
	my $TicketTypeFilter = $ConfigObject->Get('Ticket::Frontend::'.$Self->{Action})->{'TicketTypeFilter'};
	my @TypeIDs;
	foreach my $TypeName (@{$TicketTypeFilter})
	{
    ...
    if ( !$AllTicketsTotal ) {
        ...
        my $CustomTexts = $ConfigObject->Get('Ticket::Frontend::'.$Self->{Action}.'CustomEmptyText');
        ...
        if ( ref $CustomTexts eq 'HASH' ) {
            ...
            if (
                ...
                eq 'HASH'
                && defined $ConfigObject->Get('Ticket::Frontend::'.$Self->{Action}.'CustomEmptyText')
                ->{Button}
                )
            {
                $LayoutObject->Block(
        ...
        my $Owner = $ConfigObject->Get('Ticket::Frontend::'.$Self->{Action})->{Owner};
        my $Queue = $ConfigObject->Get('Ticket::Frontend::'.$Self->{Action})->{Queue};
        ...
        my $DynamicFieldFilter = $ConfigObject->Get("Ticket::Frontend::$Self->{Action}")->{DynamicField};
	...
	$Output .= $LayoutObject->Notify(
        Priority => 'Warning',
        Info => 'Selected Ticket Type : '.join( ', ', @{$TicketTypeFilter} ),
    );
...
sub ShowTicketStatus {
    ...
    my $Subject;
    my $ConfigObject          = $Kernel::OM->Get('Kernel::Config');
    my $SmallViewColumnHeader = $ConfigObject->Get('Ticket::Frontend::'.$Self->{Action})->{ColumnHeader};
